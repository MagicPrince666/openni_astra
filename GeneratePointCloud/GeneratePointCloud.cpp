...
#define DEVICE_PID_DCW 0x0659
#define DEVICE_PID_DW 0x065a
...
{
	uint16_t pid = Device.getDeviceInfo().getUsbProductId();
	if (pid == DEVICE_PID_DCW || pid == DEVICE_PID_DW)
	{
		OBCameraParamsData obCameraParamsData;
		int dataSize = sizeof(OBCameraParamsData);
		memset(&obCameraParamsData, 0, sizeof(obCameraParamsData));
		obCameraParamsData.depthRes = XN_CAMERA_PARAMS_DEPTH_RES_640_480;
		obCameraParamsData.colorRes = XN_CAMERA_PARAMS_COLOR_RES_DEFAULT;
		openni::Status rc = Device.getProperty(openni::OBEXTENSION_ID_CAM_PARAMS, (uint8_t *)&obCameraParamsData, &dataSize);
		...
		OBCameraParams cameraParam = obCameraParamsData.params;
...
void analyzeFrame(const VideoFrameRef& frame)
{
	...
	std::string dir = "PointCloud";
	MKDIR(dir.c_str());

	std::stringstream filename;

#ifdef PLATFORM_WIN
	filename << "PointCloud\\pointcloud_";
#else
	filename << "PointCloud//pointcloud_";
#endif
